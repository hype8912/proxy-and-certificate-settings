...
| <a name="apt"></a>apt | Advanced Package Tool | Debian | debian:stable-slim<br>ubuntu:latest | 27MB<br>28MB | <pre lang="bash">echo 'Acquire::http::Proxy "$HTTP_PROXY";' > /etc/apt/apt.conf.d/00proxy&#13;echo 'Acquire::https::Proxy "$HTTPS_PROXY";' >> /etc/apt/apt.conf.d/00proxy</pre> | |
...
| composer | Composer (PHP)[^composer] | Alpine | composer:latest | 71MB | | <pre lang="bash">export CGI_HTTP_PROXY="$SSL_CERT_FILE"</pre> |
| <a name="conda"></a>conda | Conda | Debian | continuumio/miniconda3:latest | 198MB | `conda config --set proxy_servers.http "$HTTP_PROXY"`<br>`conda config --set proxy_servers.https "$HTTPS_PROXY"` | `conda config --set ssl_verify "$SSL_CERT_FILE"` |
...
| <a name="lein"></a>lein | Leiningen | Debian | clojure:latest | 287MB | | See [java](https://gist.github.com/hype8912/40116cb5e1051fc1a9b29ea54e6c2139#java). |
...
| pear | PEAR[^pear] | Alpine<br>Debian | php:alpine<br>php:latest | 40MB<br>178MB | <pre lang="bash">pear config-set http_proxy "$HTTP_PROXY"&#13;pear config-set https_proxy "$HTTPS_PROXY"</pre> | |
| <a name="pip"></a>pip | pip (python) | Alpine<br>Debian | python:alpine<br>python:slim | 16MB<br>42MB | | <pre lang="bash">export CURL_CA_BUNDLE="$SSL_CA_CERT"&#13;export PIP_CERT="SSL_CERT_FILE"&#13;export REQUESTS_CA_BUNDLE="SSL_CERT_FILE"</pre> |
...
| port | MacPorts | MacOS | | | | |
| <a name="rpm"></a>rpm | RPM Package Manager | Red Hat | almalinux:latest<br>centos8:latest<br>quay.io/centos/centos:stream10<br>eurolinux/eurolinux-9:latest<br>fedora:latest<br>rockylinux:9<br>oraclelinux:9<br>redhat/ubi9:latest[^ubi] | 75MB<br>80MB<br>105MB<br>61MB<br>56MB<br>61MB<br>101MB<br>84MB | | |
...
| <a name="uv"></a>uv | uv (python) | Alpine | ghcr.io/astral-sh/uv:alpine | 45MB | | See [pip](#pip). |
...
| <a name="yarn"></a>yarn | Yet Another Resource Negotiator (node)[^yarn] | Alpine | jitesoft/node-yarn:lts-slim | 60MB | See [npm](#npm).<pre lang="bash">yarn config set httpProxy "$HTTP_PROXY"&#13;yarn config set httpsProxy "$HTTPS_PROXY"</pre> | See [npm](#npm).<pre lang="bash">yarn config set httpsCaFilePath "$SSL_CERT_FILE"</pre> |
