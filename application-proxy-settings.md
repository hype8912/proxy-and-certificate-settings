...
|:---:|:---:|---|:---:|---|---|
| | Apache Karaf | apache/karaf:latest | 121MB | | <pre lang="bash" style="white-space:pre-wrap;">export EXTRA_JAVA_OPTS="-Djavax.net.ssl.trustStore=$SSL_KEYSTORE_FILE -Djavax.net.ssl.trustStorePassword={Password}"</pre> |
...
| | GitLab Runner Service | gitlab/gitlab-runner:latest | 106MB | `/etc/systemd/system/gitlab-runner.service.d/http-proxy.conf`<br><pre lang="ini" style="white-space:pre-wrap;">[Service]&#13;Environment="HTTP_PROXY=$HTTP_PROXY"&#13;Environment="HTTPS_PROXY=$HTTPS_PROXY"&#13;Environment="NO_PROXY=$NO_PROXY"</pre>`/etc/gitlab-runner/config.toml`<pre lang="yaml" style="white-space:pre-wrap;">[[runners]]&#13;pre_get_sources_script = "git config --global http.proxy $HTTP_PROXY; git config --global https.proxy $HTTPS_PROXY"&#13;environment = ["https_proxy=$HTTPS_PROXY", "http_proxy=$HTTP_PROXY", "HTTPS_PROXY=$HTTPS_PROXY", "HTTP_PROXY=$HTTP_PROXY"]</pre> | `/etc/gitlab-runner/config.toml`<pre lang="yaml" style="white-space:pre-wrap;">[runners.docker]&#13;volumes = ["/path/to-ca-cert-dir/ca.crt:/etc/gitlab-runner/certs/ca.crt:ro"] [^gitlab_runner]</pre> |
...
| java<a name="java"></a> | OpenJDK | openjdk:25-slim | 233MB | <pre lang="bash" style="white-space:pre-wrap;">export JAVA_OPTS="$JAVA_OPTS -Dhttp.proxyHost=$HTTP_PROXY_HOST -Dhttp.proxyPort=$HTTP_PROXY_PORT -Dhttps.proxyHost=$HTTPS_PROXY_HOST -Dhttps.proxyPort=$HTTPS_PROXY_PORT"</pre> | Linux and Darwin:<br><pre lang="bash" style="white-space:pre-wrap;">keytool -importkeystore -srckeystore $SSL_KEYSTORE_FILE -destkeystore $JAVA_HOME/lib/security/cacerts</pre>Windows:<pre lang="bash" style="white-space:pre-wrap;">keytool -importkeystore -srckeystore %SSL_KEYSTORE_FILE% -destkeystore %JAVA_HOME%\lib\security\cacerts</pre> |
...
| | Periscope Authenticator | | | <pre lang="bash" style="white-space:pre-wrap;">git config --global lfs.transfer.enablehrefrewrite true&#13;git config --global url."http://localhost:[Local Port]".insteadOf"[Git Host URL]"</pre> | |
| subscription-manager | RHEL Subscription Manager | redhat/ubi9:latest | 84MB | <pre lang="bash" style="white-space:pre-wrap;">subscription-manager config --server.proxy_hostname $HTTP_PROXY_HOST --server.proxy_port $HTTP_PROXY_PORT</pre> | |
...
| | TerraTeam | ghcr.io/terrateamio/terrat-oss:latest | | | <pre lang="yaml" style="white-space:pre-wrap;">hooks:&#13;  all:&#13;    pre:&#13;      - type: run&#13;        cmd: ['sh', '-c', 'echo "$SELF_SIGNED_CERT" > $SSL_CA_CERT && update-ca-certificates'] [^terrateam]</pre> |
